...
cairo_surface_t *cairo_image_surface_create_from_jpeg(const char *filename)
{
   ...
   long st_size = ftell(infile);
   fseek(infile, 0, SEEK_SET);

   // allocate memory
   if ((data = malloc(st_size)) == NULL)
      return cairo_image_surface_create(CAIRO_FORMAT_INVALID, 0, 0);

   // read data
   if ((long)fread(data, 1, st_size, infile) < st_size)
      return cairo_image_surface_create(CAIRO_FORMAT_INVALID, 0, 0);

   fclose(infile);

   return cairo_image_surface_create_from_jpeg_mem(data, st_size);
}
