...
//--------------------------------------------------
ofBuffer ofBufferFromFile(const used_filesystem::path & path, bool binary){
	ofFile f(path,ofFile::ReadOnly, binary);
	return ofBuffer(f);
}

//--------------------------------------------------
bool ofBufferToFile(const used_filesystem::path & path, const ofBuffer& buffer, bool binary){
...
ofFile::ofFile(const used_filesystem::path & path, Mode mode, bool binary)
:mode(mode)
,binary(true){
...
//------------------------------------------------------------------------------------------------------------
bool ofFile::open(const used_filesystem::path & _path, Mode _mode, bool binary){
	close();
	myFile = used_filesystem::path(ofToDataPath(_path.string()));
	return openStream(_mode, binary);
}

//------------------------------------------------------------------------------------------------------------
bool ofFile::openFromCWD(const used_filesystem::path & _path, Mode _mode, bool binary){
	close();
	myFile = used_filesystem::path(_path.string());
	return openStream(_mode, binary);
}
...
bool ofFile::changeMode(Mode _mode, bool binary){
	if(_mode != mode){
		string _path = path();
		close();
		myFile = used_filesystem::path(_path);
		return openStream(_mode, binary);
	}
	else{
		return true;
...
//------------------------------------------------------------------------------------------------------------
bool ofFile::create(const used_filesystem::path & path){
	bool success = false;
...
ofBuffer ofFile::readToBuffer(){
	if(myFile.string().empty() || !used_filesystem::exists(myFile)){
		return ofBuffer();
	}
...
bool ofFile::exists() const {
	...
	return used_filesystem::exists(myFile);
}
...
bool ofFile::canRead() const {
	auto perm = used_filesystem::status(myFile).permissions();
#ifdef TARGET_WIN32
	...
	if(geteuid() == info.st_uid){
		return (perm & used_filesystem::perms::owner_read) != used_filesystem::perms::none;
	}else if (getegid() == info.st_gid){
		return (perm & used_filesystem::perms::group_read) != used_filesystem::perms::none;
	}else{
		return (perm & used_filesystem::perms::others_read) != used_filesystem::perms::none;
	}
#else
	if(geteuid() == info.st_uid){
		return perm & used_filesystem::owner_read;
	}else if (getegid() == info.st_gid){
		return perm & used_filesystem::group_read;
	}else{
		return perm & used_filesystem::others_read;
	}
#endif
#endif
}
...
#if OF_USING_STD_FS
	if(geteuid() == info.st_uid){
		return (perm & used_filesystem::perms::owner_write) != used_filesystem::perms::none;
	}else if (getegid() == info.st_gid){
		return (perm & used_filesystem::perms::group_write) != used_filesystem::perms::none;
	}else{
		return (perm & used_filesystem::perms::others_write) != used_filesystem::perms::none;
	}
#else
	if(geteuid() == info.st_uid){
		return perm & used_filesystem::owner_write;
	}else if (getegid() == info.st_gid){
		return perm & used_filesystem::group_write;
	}else{
		return perm & used_filesystem::others_write;
	}
#endif
#endif
}
...
#if OF_USING_STD_FS
	if(geteuid() == info.st_uid){
		return (perm & used_filesystem::perms::owner_exec) != used_filesystem::perms::none;
	}else if (getegid() == info.st_gid){
		return (perm & used_filesystem::perms::group_exec) != used_filesystem::perms::none;
	}else{
		return (perm & used_filesystem::perms::others_exec) != used_filesystem::perms::none;
	}
#else
	if(geteuid() == info.st_uid){
		return perm & used_filesystem::owner_exe;
	}else if (getegid() == info.st_gid){
		return perm & used_filesystem::group_exe;
	}else{
		return perm & used_filesystem::others_exe;
	}
#endif
#endif
}
...
bool ofFile::isFile() const {
	return used_filesystem::is_regular_file(myFile);
}

//------------------------------------------------------------------------------------------------------------
bool ofFile::isLink() const {
	return used_filesystem::is_symlink(myFile);
}

//------------------------------------------------------------------------------------------------------------
bool ofFile::isDirectory() const {
	return used_filesystem::is_directory(myFile);
}
...
#else
#if OF_USING_STD_FS
	return used_filesystem::is_block_file(used_filesystem::status(myFile));
#else
	return used_filesystem::status(myFile).type() == used_filesystem::block_file;
#endif
#endif
}
...
void ofFile::setWriteable(bool flag){
	try{
        ...
        if(flag){
            used_filesystem::permissions(myFile,used_filesystem::perms::owner_write | used_filesystem::perms::add_perms);
        }else{
            used_filesystem::permissions(myFile,used_filesystem::perms::owner_write | used_filesystem::perms::remove_perms);
        }
#else
        if(flag){
            used_filesystem::permissions(myFile,
                                         used_filesystem::perms::owner_write,
                                         used_filesystem::perm_options::add);
        }else{
            used_filesystem::permissions(myFile,
                                         used_filesystem::perms::owner_write,
                                         used_filesystem::perm_options::remove);
        }
#endif
...
#if !OF_USING_STD_FS || (OF_USING_STD_FS && OF_USE_EXPERIMENTAL_FS)
        if(flag){
            used_filesystem::permissions(myFile,used_filesystem::perms::owner_read | used_filesystem::perms::add_perms);
        }else{
            used_filesystem::permissions(myFile,used_filesystem::perms::owner_read | used_filesystem::perms::remove_perms);
        }
#else
        if(flag){
            used_filesystem::permissions(myFile,
                                         used_filesystem::perms::owner_read,
                                         used_filesystem::perm_options::add);
        }else{
            used_filesystem::permissions(myFile,
                                         used_filesystem::perms::owner_read,
...
#   if OF_USE_EXPERIMENTAL_FS
        if(flag){
            used_filesystem::permissions(myFile, used_filesystem::perms::owner_exec | used_filesystem::perms::add_perms);
        } else{
            used_filesystem::permissions(myFile, used_filesystem::perms::owner_exec | used_filesystem::perms::remove_perms);
        }
#   else
        if(flag){
            used_filesystem::permissions(myFile,
                                         used_filesystem::perms::owner_exec,
                                         used_filesystem::perm_options::add);
        } else{
            used_filesystem::permissions(myFile,
                                         used_filesystem::perms::owner_exec,
...
#else
		if(flag){
			used_filesystem::permissions(myFile, used_filesystem::perms::owner_exe | used_filesystem::perms::add_perms);
		} else{
			used_filesystem::permissions(myFile, used_filesystem::perms::owner_exe | used_filesystem::perms::remove_perms);
		}
#endif
	}catch(std::exception & e){
...
//------------------------------------------------------------------------------------------------------------
bool ofFile::copyTo(const used_filesystem::path& _path, bool bRelativeToData, bool overwrite) const{
	auto path = _path;
	...
	try{
		...
		used_filesystem::copy_file(myFile,path);
...
//------------------------------------------------------------------------------------------------------------
bool ofFile::moveTo(const used_filesystem::path& _path, bool bRelativeToData, bool overwrite){
	auto path = _path;
	...
	try{
		...
		used_filesystem::rename(myFile,path);
		myFile = path;
		if(mode != ofFile::Reference){
...
//------------------------------------------------------------------------------------------------------------
bool ofFile::renameTo(const used_filesystem::path& path, bool bRelativeToData, bool overwrite){
...
bool ofFile::remove(bool recursive){
	...
	try{
		...
		if(recursive){
			used_filesystem::remove_all(myFile);
		}else{
			used_filesystem::remove(myFile);
		}
	}catch(std::exception & except){
...
uint64_t ofFile::getSize() const {
	try{
		return used_filesystem::file_size(myFile);
	}catch(std::exception & except){
...
bool ofFile::copyFromTo(const used_filesystem::path& pathSrc, const used_filesystem::path& pathDst, bool bRelativeToData,  bool overwrite){
	ofFile tmp;
	if( bRelativeToData ){
...
//------------------------------------------------------------------------------------------------------------
bool ofFile::moveFromTo(const used_filesystem::path& pathSrc, const used_filesystem::path& pathDst, bool bRelativeToData, bool overwrite){
	ofFile tmp;
	if( bRelativeToData ){
		tmp.open(pathSrc,ofFile::Reference);
	}else{
...
//------------------------------------------------------------------------------------------------------------
bool ofFile::doesFileExist(const used_filesystem::path& _fPath, bool bRelativeToData){
	ofFile tmp;
	if(bRelativeToData){
...
//------------------------------------------------------------------------------------------------------------
bool ofFile::removeFile(const used_filesystem::path& _path, bool bRelativeToData){
	ofFile tmp;
	if(bRelativeToData){
...
//------------------------------------------------------------------------------------------------------------
ofDirectory::ofDirectory(const used_filesystem::path & path){
	showHidden = false;
	open(path);
}

//------------------------------------------------------------------------------------------------------------
void ofDirectory::open(const used_filesystem::path & path){
	originalDirectory = ofFilePath::getPathForDirectory(path.string());
	files.clear();
	myDir = used_filesystem::path(ofToDataPath(originalDirectory));
}

//------------------------------------------------------------------------------------------------------------
void ofDirectory::openFromCWD(const used_filesystem::path & path){
	originalDirectory = ofFilePath::getPathForDirectory(path.string());
	files.clear();
	myDir = used_filesystem::path(originalDirectory);
}

//------------------------------------------------------------------------------------------------------------
void ofDirectory::close(){
	myDir = used_filesystem::path();
}
...
bool ofDirectory::create(bool recursive){
	...
	if(!myDir.string().empty()){
		try{
			if(recursive){
				used_filesystem::create_directories(myDir);
			}else{
				used_filesystem::create_directory(myDir);
			}
		}
		catch(std::exception & except){
...
bool ofDirectory::exists() const {
	return (myDir == "" || used_filesystem::exists(myDir));
}
...
string ofDirectory::getAbsolutePath() const {
	try{
		return used_filesystem::canonical(used_filesystem::absolute(myDir)).string();
	}catch(...){
		return used_filesystem::absolute(myDir).string();
	}
}
...
bool ofDirectory::isDirectory() const {
	return used_filesystem::is_directory(myDir);
}

//------------------------------------------------------------------------------------------------------------
bool ofDirectory::copyTo(const used_filesystem::path& _path, bool bRelativeToData, bool overwrite){
	auto path = _path;
	...
	if(!used_filesystem::exists(myDir)){
		ofLogError("ofDirectory") << "copyTo(): source directory does not exist";
		return false;
	}
	if(!used_filesystem::is_directory(myDir)){
	...
	for(used_filesystem::directory_iterator file(myDir); file != used_filesystem::directory_iterator(); ++file){
		auto currentPath = used_filesystem::absolute(file->path());
		auto dst = used_filesystem::path(path) / currentPath.filename();
		if(used_filesystem::is_directory(currentPath)){
...
//------------------------------------------------------------------------------------------------------------
bool ofDirectory::moveTo(const used_filesystem::path& path, bool bRelativeToData, bool overwrite){
...
//------------------------------------------------------------------------------------------------------------
bool ofDirectory::renameTo(const used_filesystem::path& path, bool bRelativeToData, bool overwrite){
	return moveTo(path, bRelativeToData, overwrite);
}

//------------------------------------------------------------------------------------------------------------
bool ofDirectory::remove(bool recursive){
	if(path().empty() || !used_filesystem::exists(myDir)){
		return false;
	}

	try{
		if(recursive){
			used_filesystem::remove_all(used_filesystem::canonical(myDir));
		}else{
			used_filesystem::remove(used_filesystem::canonical(myDir));
		}
	}catch(std::exception & except){
...
std::size_t ofDirectory::listDir(){
	...
	used_filesystem::directory_iterator end_iter;
	if ( used_filesystem::exists(myDir) && used_filesystem::is_directory(myDir)){
		for( used_filesystem::directory_iterator dir_iter(myDir) ; dir_iter != end_iter ; ++dir_iter){
...
static bool byDate(const ofFile& a, const ofFile& b) {
	auto ta = used_filesystem::last_write_time(a);
	auto tb = used_filesystem::last_write_time(b);
	return ta < tb;
}
...
//------------------------------------------------------------------------------------------------------------
bool ofDirectory::removeDirectory(const used_filesystem::path& _path, bool deleteIfNotEmpty, bool bRelativeToData){
	auto path = _path;
...
//------------------------------------------------------------------------------------------------------------
bool ofDirectory::createDirectory(const used_filesystem::path& _dirPath, bool bRelativeToData, bool recursive){
	...
	// on OSX,used_filesystem::create_directories seems to return false *if* the path has folders that already exist
	...
	if (!bDoesExistAlready){
		...
		try{
			if(!recursive){
				success = used_filesystem::create_directory(dirPath);
			}else{
				success = used_filesystem::create_directories(dirPath);
			}
		} catch(std::exception & except){
...
//------------------------------------------------------------------------------------------------------------
bool ofDirectory::doesDirectoryExist(const used_filesystem::path& _dirPath, bool bRelativeToData){
	auto dirPath = _dirPath;
	try {
		if (bRelativeToData) {
			dirPath = ofToDataPath(dirPath);
		}
		return used_filesystem::exists(dirPath) && used_filesystem::is_directory(dirPath);
	}
	catch (std::exception & except) {
...
//------------------------------------------------------------------------------------------------------------
bool ofDirectory::isDirectoryEmpty(const used_filesystem::path& _dirPath, bool bRelativeToData){
	auto dirPath = _dirPath;
	if(bRelativeToData){
		dirPath = ofToDataPath(dirPath);
	}

	if(!dirPath.empty() && used_filesystem::exists(dirPath) && used_filesystem::is_directory(dirPath)){
		return used_filesystem::directory_iterator(dirPath) == used_filesystem::directory_iterator();
	}
	return false;
}
...
//------------------------------------------------------------------------------------------------------------
string ofFilePath::addLeadingSlash(const used_filesystem::path& _path){
    auto path = _path.string();
	auto sep = used_filesystem::path("/").make_preferred();
	if(!path.empty()){
...
//------------------------------------------------------------------------------------------------------------
string ofFilePath::addTrailingSlash(const used_filesystem::path& _path){
    auto path = used_filesystem::path(_path).make_preferred().string();
	auto sep = used_filesystem::path("/").make_preferred();
	if(!path.empty()){
...
//------------------------------------------------------------------------------------------------------------
string ofFilePath::getFileExt(const used_filesystem::path& filename){
	return ofFile(filename,ofFile::Reference).getExtension();
}

//------------------------------------------------------------------------------------------------------------
string ofFilePath::removeExt(const used_filesystem::path& filename){
	return ofFilePath::join(getEnclosingDirectory(filename,false), ofFile(filename,ofFile::Reference).getBaseName());
}


//------------------------------------------------------------------------------------------------------------
string ofFilePath::getPathForDirectory(const used_filesystem::path& path){
...
//------------------------------------------------------------------------------------------------------------
string ofFilePath::removeTrailingSlash(const used_filesystem::path& _path){
    auto path = _path.string();
...
//------------------------------------------------------------------------------------------------------------
string ofFilePath::getFileName(const used_filesystem::path& _filePath, bool bRelativeToData){
	...
	return used_filesystem::path(filePath).filename().string();
}

//------------------------------------------------------------------------------------------------------------
string ofFilePath::getBaseName(const used_filesystem::path& filePath){
	return ofFile(filePath,ofFile::Reference).getBaseName();
}

//------------------------------------------------------------------------------------------------------------
string ofFilePath::getEnclosingDirectory(const used_filesystem::path& _filePath, bool bRelativeToData){
    std::string filePath = _filePath.string();
	if(bRelativeToData){
		filePath = ofToDataPath(filePath);
	}
	return addTrailingSlash(used_filesystem::path(filePath).parent_path().string());
}

//------------------------------------------------------------------------------------------------------------
bool ofFilePath::createEnclosingDirectory(const used_filesystem::path& filePath, bool bRelativeToData, bool bRecursive) {
	return ofDirectory::createDirectory(ofFilePath::getEnclosingDirectory(filePath,bRelativeToData), bRelativeToData, bRecursive);
}

//------------------------------------------------------------------------------------------------------------
string ofFilePath::getAbsolutePath(const used_filesystem::path& path, bool bRelativeToData){
	if(bRelativeToData){
		return ofToDataPath(path, true);
	}else{
		try{
			return used_filesystem::canonical(used_filesystem::absolute(path)).string();
		}catch(...){
			return used_filesystem::absolute(path).string();
		}
	}
}
...
//------------------------------------------------------------------------------------------------------------
bool ofFilePath::isAbsolute(const used_filesystem::path& path){
	return used_filesystem::path(path).is_absolute();
}

//------------------------------------------------------------------------------------------------------------
string ofFilePath::getCurrentWorkingDirectory(){
	return used_filesystem::current_path().string();
}

//------------------------------------------------------------------------------------------------------------
string ofFilePath::join(const used_filesystem::path& path1, const used_filesystem::path& path2){
	return (used_filesystem::path(path1) / used_filesystem::path(path2)).string();
}
...
string ofFilePath::makeRelative(const used_filesystem::path & from, const used_filesystem::path & to){
	auto pathFrom = used_filesystem::absolute( from );
	auto pathTo = used_filesystem::absolute( to );
	used_filesystem::path ret;
	used_filesystem::path::const_iterator itrFrom( pathFrom.begin() ), itrTo( pathTo.begin() );
	// Find common base
	for( used_filesystem::path::const_iterator toEnd( pathTo.end() ), fromEnd( pathFrom.end() ) ; itrFrom != fromEnd && itrTo != toEnd && *itrFrom == *itrTo; ++itrFrom, ++itrTo );
	// Navigate backwards in directory to reach previously found base
	for( used_filesystem::path::const_iterator fromEnd( pathFrom.end() ); itrFrom != fromEnd; ++itrFrom ){
		if( (*itrFrom) != "." ){
			ret /= "..";
		}
	}
