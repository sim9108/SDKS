...
template<typename PixelType>
static bool loadImage(ofPixels_<PixelType> & pix, const used_filesystem::path& _fileName, const ofImageLoadSettings& settings){
	ofInitFreeImage();
...
//----------------------------------------------------------------
bool ofLoadImage(ofPixels & pix, const used_filesystem::path& path, const ofImageLoadSettings &settings) {
...
//----------------------------------------------------------------
bool ofLoadImage(ofShortPixels & pix, const used_filesystem::path& path, const ofImageLoadSettings &settings) {
	return loadImage(pix, path, settings);
}
...
//----------------------------------------------------------------
bool ofLoadImage(ofFloatPixels & pix, const used_filesystem::path& path, const ofImageLoadSettings &settings) {
	return loadImage(pix, path, settings);
}
...
//----------------------------------------------------------------
bool ofLoadImage(ofTexture & tex, const used_filesystem::path& path, const ofImageLoadSettings &settings){
	ofPixels pixels;
...
//----------------------------------------------------------------
template<typename PixelType>
static bool saveImage(const ofPixels_<PixelType> & _pix, const used_filesystem::path& _fileName, ofImageQualityType qualityLevel) {
	ofInitFreeImage();
...
//----------------------------------------------------------------
bool ofSaveImage(const ofPixels & pix, const used_filesystem::path& fileName, ofImageQualityType qualityLevel){
	return saveImage(pix,fileName,qualityLevel);
}

//----------------------------------------------------------------
bool ofSaveImage(const ofFloatPixels & pix, const used_filesystem::path& fileName, ofImageQualityType qualityLevel) {
	return saveImage(pix,fileName,qualityLevel);
}

//----------------------------------------------------------------
bool ofSaveImage(const ofShortPixels & pix, const used_filesystem::path& fileName, ofImageQualityType qualityLevel) {
	return saveImage(pix,fileName,qualityLevel);
}
...
template<typename PixelType>
ofImage_<PixelType>::ofImage_(const used_filesystem::path & fileName, const ofImageLoadSettings &settings){
	width						= 0;
	height						= 0;
	bpp							= 0;
	type						= OF_IMAGE_UNDEFINED;
...
//----------------------------------------------------------
template<typename PixelType>
bool ofImage_<PixelType>::load(const used_filesystem::path& fileName, const ofImageLoadSettings &settings){
	#if defined(TARGET_ANDROID)
	ofAddListener(ofxAndroidEvents().unloadGL,this,&ofImage_<PixelType>::unloadTexture);
	ofAddListener(ofxAndroidEvents().reloadGL,this,&ofImage_<PixelType>::update);
	#endif
...
//----------------------------------------------------------
template<typename PixelType>
bool ofImage_<PixelType>::save(const used_filesystem::path& fileName, ofImageQualityType qualityLevel) const {
