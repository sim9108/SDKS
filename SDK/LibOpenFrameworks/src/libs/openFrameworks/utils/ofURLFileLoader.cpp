...
public:
	...
	ofHttpResponse saveTo(const string& url, const used_filesystem::path& path);
	int saveAsync(const string& url, const used_filesystem::path& path);
	void remove(int id);
	void clear();
	void stop();
...
ofHttpResponse ofURLFileLoaderImpl::saveTo(const string& url, const used_filesystem::path& path){
	ofHttpRequest request(url,path.string(),true);
	return handleRequest(request);
}

int ofURLFileLoaderImpl::saveAsync(const string& url, const used_filesystem::path& path){
	ofHttpRequest request(url,path.string(),true);
	requests.send(request);
	start();
	return request.getId();
...
ofHttpResponse ofURLFileLoader::saveTo(const string& url, const used_filesystem::path & path){
	return impl->saveTo(url,path);
}

int ofURLFileLoader::saveAsync(const string& url, const used_filesystem::path & path){
...
ofHttpResponse ofSaveURLTo(const string& url, const used_filesystem::path& path){
	return getFileLoader().saveTo(url,path);
}

int ofSaveURLAsync(const string& url, const used_filesystem::path& path){
