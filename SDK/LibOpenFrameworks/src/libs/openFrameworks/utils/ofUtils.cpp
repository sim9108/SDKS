...
namespace{
    ...
    string defaultDataPath(){
        ...
        try{
            return used_filesystem::canonical(ofFilePath::join(ofFilePath::getCurrentExeDir(),  "../../../data/")).string();
        }catch(...){
    ...
    #else
        try{
            return used_filesystem::canonical(ofFilePath::join(ofFilePath::getCurrentExeDir(),  "data/")).make_preferred().string();
        }catch(...){
            return ofFilePath::join(ofFilePath::getCurrentExeDir(),  "data/");
        }
    #endif
    ...
    //--------------------------------------------------
    used_filesystem::path & defaultWorkingDirectory(){
            static auto * defaultWorkingDirectory = new used_filesystem::path(ofFilePath::getCurrentExeDir());
            return * defaultWorkingDirectory;
    }

    //--------------------------------------------------
    used_filesystem::path & dataPathRoot(){
            static auto * dataPathRoot = new used_filesystem::path(defaultDataPath());
            return *dataPathRoot;
    }
}
...
namespace priv{
	void initutils(){
        defaultWorkingDirectory() = used_filesystem::absolute(used_filesystem::current_path());
...
bool ofRestoreWorkingDirectoryToDefault(){
    try{
        used_filesystem::current_path(defaultWorkingDirectory());
        return true;
    }catch(...){
        return false;
    }
...
//--------------------------------------------------
void ofSetDataPathRoot(const used_filesystem::path& newRoot){
	dataPathRoot() = newRoot;
}

//--------------------------------------------------
string ofToDataPath(const used_filesystem::path & path, bool makeAbsolute){
...
#ifdef TARGET_WIN32
	if (defaultWorkingDirectory() != used_filesystem::current_path()) {
	...
	used_filesystem::path inputPath(path);
	used_filesystem::path outputPath;

	// if path is already absolute, just return it
	if (inputPath.is_absolute()) {
		try {
            auto outpath = used_filesystem::canonical(inputPath).make_preferred();
            if(used_filesystem::is_directory(outpath) && hasTrailingSlash){
    ...
    auto relativeDirDataPath = ofFilePath::makeRelative(used_filesystem::current_path().string(),dataPath.string());
	...
	if(makeAbsolute){
	    ...
	    try {
            auto outpath = used_filesystem::canonical(used_filesystem::absolute(outputPath)).make_preferred();
            if(used_filesystem::is_directory(outpath) && hasTrailingSlash){
                return ofFilePath::addTrailingSlash(outpath.string());
            }else{
                return outpath.string();
            ...
            return used_filesystem::absolute(outputPath).string();
	    }
	}else{
