...
inline ofJson ofLoadJson(const used_filesystem::path& filename){
	ofJson json;
...
inline bool ofSaveJson(const used_filesystem::path& filename, const ofJson & json){
...
/// \param json The Json to save.
/// \returns true if the json was saved successfully.
inline bool ofSavePrettyJson(const used_filesystem::path& filename, const ofJson & json){
    ofFile jsonFile(filename, ofFile::WriteOnly);
    try{
