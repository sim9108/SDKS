...
#	include <boost/filesystem.hpp>
	namespace used_filesystem = boost::filesystem;
#endif
...
ofBuffer ofBufferFromFile(const used_filesystem::path & path, bool binary=true);
...
/// split at endline characters automatically
bool ofBufferToFile(const used_filesystem::path & path, const ofBuffer& buffer, bool binary=true);
...
public:
    ...
    static std::string getFileExt(const used_filesystem::path& filename);
	
	/// Remove extension from a filename, ie. "duck.jpg" ->"duck".
	///
	/// \param filename file path
	/// \returns filename without extension
    static std::string removeExt(const used_filesystem::path& filename);
	
	/// Prepend path with a slash, ie. "images" -> "/images".
	///
	/// \param path file or directory path
	/// \returns slah + path
    static std::string addLeadingSlash(const used_filesystem::path& path);
	
	/// Append path with a slash, ie. "images" -> "images/".
	///
	/// \param path directory path
	/// \returns path + slash
    static std::string addTrailingSlash(const used_filesystem::path& path);
    ...
    static std::string removeTrailingSlash(const used_filesystem::path& path);
    ...
    static std::string getPathForDirectory(const used_filesystem::path& path);
    ...
    static std::string getAbsolutePath(const used_filesystem::path& path, bool bRelativeToData = true);
    ...
    static bool isAbsolute(const used_filesystem::path& path);
	...
	/// "../../"
	/// \returns filename
    static std::string getFileName(const used_filesystem::path& filePath, bool bRelativeToData = true);
	...
	/// \param filePath file path
	/// \returns basename
    static std::string getBaseName(const used_filesystem::path& filePath);
    ...
    static std::string getEnclosingDirectory(const used_filesystem::path& filePath, bool bRelativeToData = true);
    ...
    static bool createEnclosingDirectory(const used_filesystem::path& filePath, bool bRelativeToData = true, bool bRecursive = true);
	...
	/// \returns joined path
    static std::string join(const used_filesystem::path& path1, const used_filesystem::path& path2);
    ...
    static std::string makeRelative(const used_filesystem::path & from, const used_filesystem::path & to);
};
...
public:
	...
	ofFile(const used_filesystem::path & path, Mode mode=ReadOnly, bool binary=true);
	...
	bool open(const used_filesystem::path & path, Mode mode=ReadOnly, bool binary=true);
	...
	/// split at endline characters automatically
	/// \returns true if the path was opened
	bool openFromCWD(const used_filesystem::path & path, Mode mode=ReadOnly, bool binary=true);
	...
	/// \param path file path
	/// \returns true if the file was created
	bool create(const used_filesystem::path & path);
	...
	bool copyTo(const used_filesystem::path& path, bool bRelativeToData = true, bool overwrite = false) const;
	...
	/// directory at the new path
	/// \returns true if the copy was successful
	bool moveTo(const used_filesystem::path& path, bool bRelativeToData = true, bool overwrite = false);
	...
	/// directory at the new path
	/// \returns true if the copy was successful
	bool renameTo(const used_filesystem::path& path, bool bRelativeToData = true, bool overwrite = false);
	...
	operator used_filesystem::path(){
		return myFile;
	}

	operator const used_filesystem::path() const{
		return myFile;
	}
	...
	/// directory at the new path
	/// \returns true if the copy was successful
	static bool copyFromTo(const used_filesystem::path& pathSrc, const used_filesystem::path& pathDst, bool bRelativeToData = true,  bool overwrite = false);
	...
	static bool moveFromTo(const used_filesystem::path& pathSrc, const used_filesystem::path& pathDst, bool bRelativeToData = true, bool overwrite = false);
	...
	static bool doesFileExist(const used_filesystem::path& fPath,  bool bRelativeToData = true);
	...
	/// "../../"
	/// \returns true if the path was removed successfully
	static bool removeFile(const used_filesystem::path& path, bool bRelativeToData = true);

private:
	bool isWriteMode();
	bool openStream(Mode _mode, bool binary);
	void copyFrom(const ofFile & mom);
	used_filesystem::path myFile;
	Mode mode;
	bool binary;
};
...
public:
	...
	///
	/// \param path directory path
	ofDirectory(const used_filesystem::path & path);

	/// Open a directory path, clears the current file list.
	///
	/// \param path directory path
	void open(const used_filesystem::path & path);
	...
	///
	/// \param path directory path
	void openFromCWD(const used_filesystem::path & path);
	...
	/// directory at the new path
	/// \returns true if the copy was successful
	bool copyTo(const used_filesystem::path& path, bool bRelativeToData = true, bool overwrite = false);
	...
	static bool createDirectory(const used_filesystem::path& dirPath, bool bRelativeToData = true, bool recursive = false);
	...
	/// directories
	static bool isDirectoryEmpty(const used_filesystem::path& dirPath, bool bRelativeToData = true );
	...
	static bool doesDirectoryExist(const used_filesystem::path& dirPath, bool bRelativeToData = true);
	...
	/// are *not* in the data directory
	/// \returns true if the path was removed successfully
	static bool removeDirectory(const used_filesystem::path& path, bool deleteIfNotEmpty,  bool bRelativeToData = true);
