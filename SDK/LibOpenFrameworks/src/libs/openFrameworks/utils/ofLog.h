...
void ofLogToFile(const used_filesystem::path & path, bool append=false);
...
public:
    ...
    ofFileLoggerChannel(const used_filesystem::path & path, bool append);
	...
	/// \param path The file path for the log file.
	/// \param append True if the log data should be added to an existing file.
    void setFile(const used_filesystem::path & path,bool append=false);
