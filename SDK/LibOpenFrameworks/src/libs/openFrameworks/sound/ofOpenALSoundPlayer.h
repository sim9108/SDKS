...
class ofOpenALSoundPlayer : public ofBaseSoundPlayer, public ofThread {
	...
	public:
        ...
        bool load(const used_filesystem::path& fileName, bool stream = false);
		void unload();
		void play();
		void stop();
	...
	private:
        ...
        bool sfReadFile(const used_filesystem::path& path,std::vector<short> & buffer,std::vector<float> & fftAuxBuffer);
        bool sfStream(const used_filesystem::path& path,std::vector<short> & buffer,std::vector<float> & fftAuxBuffer);
#ifdef OF_USING_MPG123
        bool mpg123ReadFile(const used_filesystem::path& path,std::vector<short> & buffer,std::vector<float> & fftAuxBuffer);
        bool mpg123Stream(const used_filesystem::path& path,std::vector<short> & buffer,std::vector<float> & fftAuxBuffer);
#endif

        bool readFile(const used_filesystem::path& fileName,std::vector<short> & buffer);
        bool stream(const used_filesystem::path& fileName, std::vector<short> & buffer);
