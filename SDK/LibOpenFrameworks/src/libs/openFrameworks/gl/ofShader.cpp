...
//--------------------------------------------------------------
bool ofShader::load(const used_filesystem::path& shaderName) {
	return load(shaderName.string() + ".vert", shaderName.string() + ".frag");
}

//--------------------------------------------------------------
bool ofShader::load(const used_filesystem::path& vertName, const used_filesystem::path& fragName, const used_filesystem::path& geomName) {
...
//--------------------------------------------------------------
bool ofShader::loadCompute(const used_filesystem::path& shaderName) {
...
//--------------------------------------------------------------
bool ofShader::setupShaderFromFile(GLenum type, const used_filesystem::path& filename) {
...
//--------------------------------------------------------------
ofShader::Source ofShader::sourceFromFile(GLenum type, const used_filesystem::path& filename) {
	ofBuffer buffer = ofBufferFromFile(filename);
	// we need to make absolutely sure to have an absolute path here, so that any #includes
	// within the shader files have a root directory to traverse from.
	string absoluteFilePath = ofFilePath::getAbsolutePath(filename, true);
...
//--------------------------------------------------------------
string ofShader::parseForIncludes( const string& source, const used_filesystem::path& sourceDirectoryPath) {
	vector<string> included;
	return parseForIncludes( source, included, 0, sourceDirectoryPath);
}

//--------------------------------------------------------------
string ofShader::parseForIncludes( const string& source, vector<string>& included, int level, const used_filesystem::path& sourceDirectoryPath) {
