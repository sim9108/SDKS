...
class ofShader {
	...
	struct Source{
		Source(GLuint type, const std::string & source, const used_filesystem::path & directoryPath)
			:type(type)
			,source(source)
		...
		used_filesystem::path directoryPath;
		std::map<std::string, int>   intDefines;
		std::map<std::string, float> floatDefines;
	};
...
public:
	...
	bool load(const used_filesystem::path& shaderName);
	bool load(const used_filesystem::path& vertName, const used_filesystem::path& fragName, const used_filesystem::path& geomName="");
#if !defined(TARGET_OPENGLES) && defined(glDispatchCompute)
	bool loadCompute(const used_filesystem::path& shaderName);
#endif
	...
	bool setupShaderFromFile(GLenum type, const used_filesystem::path& filename);
...
private:
	...
	ofShader::Source sourceFromFile(GLenum type, const used_filesystem::path& filename);
	...
	static std::string parseForIncludes( const std::string& source, const used_filesystem::path& sourceDirectoryPath = "");
	static std::string parseForIncludes( const std::string& source, std::vector<std::string>& included, int level = 0, const used_filesystem::path& sourceDirectoryPath = "");
